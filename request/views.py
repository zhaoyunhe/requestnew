...
def step_add_data(request):
    ...
    #调试GET接口
    '''
    url = "http://192.168.100.65:8325/business/metadata/city/increment"
    apiKey='4a7f6772c52711e78e68005056aa19c3'
    regex = r"\${.*?}"
    headers = re.sub(regex, apiKey, headers)
    headers = json.loads(headers)
    params = json.loads(params)
    responseJson = get(url=url, params=params, headers=headers)
    print(responseJson)
    '''
        ...
        if method=="get" or method=="postform":
            if status != None:
                Step.objects.create(step_name=step_name,step_desc=step_desc,steplevel=steplevel,method=method, \
                                    headers=headers,params=params,assert_response=asserts,status=1,case=case)
            else:
                Step.objects.create(step_name=step_name, step_desc=step_desc, steplevel=steplevel, method=method, \
                                    headers=headers, params=params, assert_response=asserts, status=0, case=case)
        elif method=="postbody":
            if status != None:
                Step.objects.create(step_name=step_name,step_desc=step_desc,steplevel=steplevel,method=method, \
                                    headers=headers,params=paramsbody,assert_response=asserts,status=1,case=case)
            else:
                Step.objects.create(step_name=step_name, step_desc=step_desc, steplevel=steplevel, method=method, \
                                    headers=headers, params=paramsbody, assert_response=asserts, status=0, case=case)

    #新增和编辑时的用例名
    contacts = get_firstPage(Step)
    case_names = get_case_name()
    return render(request, "./main/step.html", {"steps": contacts,"case_names": case_names,"isRepeat":isRepeat})
...
def step_search_name(request):
    ...
    return render(request, "./main/step.html", {"steps": contacts, "case_names": case_names,"select":select,"step_name":step_name,\
                                                "steplevel":steplevel,"selectcase_name":case_name,"selectmethod":method})
...
def get_py_data(case_ids,testcasedir):
    for case_id in case_ids:
        ...
        # 得到外键数据
        case = Case.objects.get(id=case_id)
        step_list_data=Step.objects.filter(case=case,status=1).values("id","step_name","method","params","headers","files","assert_response")
